...
@pytest.fixture
def service():
    return ReadingLogService(
        mock_repo, ReadingLogSchema, mock_user_repo, mock_book_repo
    )
...
class TestReadingLogAuthorize:
    ...
    def test_raises_forbidden_error_on_wrong_user(self, service: ReadingLogService):
...
class TestReadingLogFilters:
    def test_returns_reading_log_filter(self, service: ReadingLogService):
        filters = service._get_filters(
            ReadingLogQueryParams(limit=5, offset=10, book_title="meditations")
        )
